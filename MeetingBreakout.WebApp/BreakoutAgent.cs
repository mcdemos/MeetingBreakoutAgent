...
namespace MeetingBreakout.WebApp {
  public class BreakoutAgent(ISharePointService sharePointService, IRoomService roomService, ITeamsService teamsService) : TeamsActivityHandler {
  ...
  private readonly ITeamsService _teamsService = teamsService;
    ...
    protected override async Task OnMembersAddedAsync(IList<ChannelAccount> membersAdded, ITurnContext<IConversationUpdateActivity> turnContext, CancellationToken cancellationToken) {
      foreach (var member in membersAdded) {
        if (member.Id != turnContext.Activity.Recipient.Id) {
          try {
            var participant = await _teamsService.GetMeetingParticipantAsync(turnContext, meetingId: turnContext.Activity.Conversation.Id, participantId: member.Id, cancellationToken: cancellationToken);
    ...
    protected override async Task OnMessageActivityAsync(ITurnContext<IMessageActivity> turnContext, CancellationToken cancellationToken) {
      if (turnContext.Activity.Value != null) {
        ...
        if (value != null && value["option"] != null) {
          ...
          var member = await _teamsService.GetMemberAsync(turnContext, turnContext.Activity.From.Id, cancellationToken);
    ...
    private async Task NotifyOrganizersAsync(ITurnContext turnContext, string assignedUser, string roomLink, CancellationToken cancellationToken) {
      try {
        ...
        var members = await _teamsService.GetPagedMembersAsync(turnContext, pageSize: 50, cancellationToken: cancellationToken);

        foreach (var member in members.Members) {
          try {
            // Check if Organizer
            var participant = await _teamsService.GetMeetingParticipantAsync(turnContext, participantId: member.Id, cancellationToken: cancellationToken);
