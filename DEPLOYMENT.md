...
## 5. Configure Storage and Role Assignments

The bot uses Azure Table Storage to manage room state.

### Assign Role to App Service Managed Identity
If running on Azure, the App Service needs permission to access the Table Storage.
...
### Create Table and Seed Rooms
You need to create the table `BreakoutRooms` and populate it with your pre-created meeting links.
...
# Create Table
az storage table create --name BreakoutRooms --connection-string $CONNECTION_STRING

# Seed Rooms (Example)
# PartitionKey = Option (1, 2, 3)
# RowKey = Meeting ID (Conversation ID of the breakout room)
# MeetingLink = The join URL

# Room for Option 1
az storage entity insert --table-name BreakoutRooms --connection-string $CONNECTION_STRING \
  --entity PartitionKey=1 RowKey="MeetingId_1_A" Status=Free MeetingLink="https://teams.microsoft.com/l/meetup-join/..."

# Room for Option 2
az storage entity insert --table-name BreakoutRooms --connection-string $CONNECTION_STRING \
  --entity PartitionKey=2 RowKey="MeetingId_2_A" Status=Free MeetingLink="https://teams.microsoft.com/l/meetup-join/..."
